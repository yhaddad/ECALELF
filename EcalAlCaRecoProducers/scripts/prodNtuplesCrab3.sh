...
crab3File=tmp/ntuple_crab3_cfg.py
CRABVERSION=3 #2
FROMCRAB3=0
JOBINDEX=""
ISPRIVATE=0
...
EOF
    ...
    echo "[Yacine]  Creating crab2 file"
    cat >> ${crabFile} <<EOF
...
EOF
    ...
    echo "[Yacine] This is the crab3 file :: ${crab3File}"
    cat > ${crab3File} <<EOF
...
config.JobType.pyCfgParams = [ 'tagFile=${TAGFILE}', 'skim=${SKIM}','type=${TYPE}','doTreeOnly=1','doTree=${DOTREE}', 'jsonFile=${JSONFILE}', 'isCrab=1','isPrivate=${ISPRIVATE}', 'bunchSpacing=${BUNCHSPACING}']
